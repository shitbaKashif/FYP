    ...
    try {
      ...
      if (storedData) {
        ...
        import('./DataPU').then(({ processChartData }) => {
          ...
          // Format for chord diagram if needed
      ...
      svg.selectAll("path")
        ...
        .on("mousemove", function(event, d) {
          ...
          if (d.source && d.target) {
            tooltipText = `${names[d.source.index]} → ${names[d.target.index]}: ${d.source.value.toFixed(2)}`;
          } else {
            tooltipText = `${names[d.index]}: ${chartData[d.index].value}`;
          }
